...
public class JacocoResultParserTest {
    ...
    public void testGetJacocoResults() throws Exception {
        ...
        assertEquals(result.get("BRANCH").toString(), "BRANCH:100.00%");
        assertEquals(result.get("INSTRUCTION").toString(), "INSTRUCTION:100.00%");
        assertEquals(result.get("LINE").toString(), "LINE:100.00%");
        assertEquals(result.get("COMPLEXITY").toString(), "COMPLEXITY:10.00");
        assertEquals(result.get("METHOD").toString(), "METHOD:100.00%");
        assertEquals(result.get("CLASS").toString(), "CLASS:100.00%");
    }
}
